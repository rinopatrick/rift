...
use crate::db::query::{ColumnInfo, QueryResult};
use crate::schema::{ColumnInfo as SchemaColumn, SchemaInfo, TableInfo};
...
impl DriverWrapper {
    ...
    pub async fn get_schema(&self) -> Result<Vec<SchemaInfo>, String> {
        match self {
            DriverWrapper::Postgres(pool) => {
                ...
                crate::schema::get_schemas(&client)
                    .await
                    .map_err(|e| e.to_string())
            }
            DriverWrapper::Mysql(pool) => {
                ...
                let db_result: Vec<mysql_async::Row> = conn
                ...
                let table_rows: Vec<mysql_async::Row> =
                    conn.query(&tables_sql).await.map_err(|e| e.to_string())?;
                ...
                for table_row in table_rows {
                    ...
                    let col_rows: Vec<mysql_async::Row> =
                        conn.query(&cols_sql).await.map_err(|e| e.to_string())?;
                    let mut columns = Vec::new();
                    for col_row in col_rows {
                        let col_name: String = col_row.get(0).unwrap_or_default();
                        let data_type: String =
                            col_row.get(1).unwrap_or_else(|| "TEXT".to_string());
                        let is_nullable: String =
                            col_row.get(2).unwrap_or_else(|| "YES".to_string());
    ...
    pub async fn get_foreign_keys(&self) -> Result<Vec<crate::schema::ForeignKeyInfo>, String> {
        match self {
            DriverWrapper::Postgres(pool) => {
                let client = pool.get().await.map_err(|e| e.to_string())?;
                crate::schema::get_foreign_keys(&client)
                    .await
                    .map_err(|e| e.to_string())
            }
            DriverWrapper::Mysql(pool) => {
                ...
                Ok(rows
                    .iter()
                    .map(|r| crate::schema::ForeignKeyInfo {
    ...
    pub async fn update_cell(
        ...
        match self {
            ...
            DriverWrapper::Mysql(pool) => {
                ...
                conn.exec_drop(&sql, (value, pk_value))
                    .await
                    .map_err(|e| e.to_string())?;
                Ok(())
            }
            DriverWrapper::Sqlite(path) => {
                ...
                task::spawn_blocking(move || {
                    ...
                    conn.execute(&sql, rusqlite::params![value.as_deref(), pk_value.as_str()])
                        .map_err(|e| e.to_string())?;
                    Ok(())
                })
                .await
